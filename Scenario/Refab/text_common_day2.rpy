label alt_day2_begin:
    ...
    dreamgirl "Ну-ну. Теперь-то, с одеждой в стирке, сам обряженный как записной клоун, разве что без галстука, уйдёшь ты очень далеко и добьёшься очень многого."  # при if not (herc or loki) and not alt_day1_sl_conv and (alt_day_binder != 1): он в галстуке, хоть и криво повязанном
    ...
    "Я щедро плеснул льдом в лицо, и опять заорал. От обжигающе холодной воды перехватило дыхание, а зубы будто сами заскрипели."
...
label alt_day2_bf:
    ...
    menu:
        "Сесть со Славей":
            ...
            sl "Разумеется, собираюсь! Хотя бы до площади тебя провожу, там выберем, куда тебе идти в первую очередь. А мне ещё надо будет на площади прибраться."
...
label alt_day2_inmusic:
    ...
    "Просторное помещение, Шуберт, Бах и Лист в рамочках на стенах, рояль, пюпитр под вокалиста, микрофон — здесь явно пели, и делали это с удовольствием."
    ...
    "Девушка тянулась к гармошке, вытянувшись всем телом, однако всё равно немного не доставала, и ей приходилось все дальше и дальше забираться под рояль."
    ...
    if alt_day2_rendezvous == 1:
        ...
        "Стоящая рядом Славя чувствовала себя немного неловко, но постаралась сделать вид, будто ничего не произошло."
        "Ну и ладно. Я тоже сделаю такой вид. В конце концов, это же не я виноват, что…"
    elif alt_day2_rendezvous == 3:
    ...
    if alt_day_binder != 1:
        if persistent.mi_dj_true:
            ...
            th "Я хорошо знал всего одну азиаточку — и это явно была не Риоко."
            me "Здравствуй."
...
label alt_day2_event_camp_entrance1:
    if been_there_alt1()==1:
        ...
        "Факт того, что отсюда до ближайшей самой захудалой деревни несколько сотен километров по трассам вторичных направлений. Сбежать отсюда? Я так не думаю."
        "Откуда я это знаю?"
        "А знаю и всё."
...
label alt_day2_event_library1:
    ...
    "«БК», — приглядевшись, прочитал я. Марка известная. Присутствующие молчаливо ждали моего вступления."
    if alt_day2_rendezvous == 1:
...
label alt_day2_mz:
    ...
    mz "Ну же!"
    "Её тон был не из приятных и ждать она явно не собиралась."
...
label alt_day2_event_medic_house1:
    ...
    if alt_day2_rendezvous == 2:
        ...
        "Ну извини, не лезет в меня геркулесовая каша."
...
label alt_day2_dubstep2:
    ...
    stop music fadeout 3
    play music polyhymnia_intro fadein 3
    us "Фе-е-е-е!"
    ...
    if (alt_day2_rendezvous == 2) and alt_day1_us_shotted and alt_day2_med_done:
        ...
        "Она улыбнулась и, хлопнув Ульяну по плечу, умчалась вперёд. И снова, как и тогда, не было в её улыбки ни наглости, ни присущего ей высокомерия. Просто обычная улыбка обычной девочки, которая на несколько секунд забыла о том, что надо носить маску."
        hide dv with dissolve
    ...
    if alt_day2_rendezvous == 2:
        ...
        if alt_day2_necessary_done > 1:
            ...
            if alt_day2_rendezvous == 5:
                "Судя по сметённым повсюду кучкам мусора, уборка только началась."
                th "Может, имеет смысл быстренько слиться, пока меня не привлекли в добровольно-принудительном режиме?"
                return
            if alt_day2_rendezvous == 22:
...
label alt_day2_event_boat_station1:
    ...
    "Место само поневоле настраивает на романтический лад. Тишина, плеск воды и отблески солнца, сотней зайчиков прыгающих по бору."
...
label alt_day2_dinner:
    ...
    "Вскоре к столовой подошёл и я. Ольга Дмитриевна, как и положено вожатой, уже караулила у входа, при этом вид имела подозрительно цветущий и отдохнувший. Видели мы… Их работу."
...
label alt_day2_siesta:
    ...
    th "Сами в свою очередь игнорируют такие, казалось бы, маловажные и несложные вопросы, как  название ближайших населённых пунктов…"
    ...
    if alt_day2_rendezvous == 1:
        ...
        "Чтобы встретить её через двадцать лет, оказавшись в прошлом."
        stop sound_loop fadeout 2
        stop ambience fadeout 2
    else:
        window hide
...
label alt_day2_tournament:
    ...
    mt "Ладно, вы все организуете и проведёте. Сами? От меня ничего не потребуется?"
...
label alt_day2_walk_sl:
    ...
    "С пляжа донесся детский многоголосый смех, и я с сожалением посмотрел в ту сторону."
...
label alt_day2_ultim:
    ...
    "Мне бы не удалось пройти — она встала между мной и дверью, а в её тоне явно проявились угрожающие нотки."
    if alt_day2_walk == 2:
    ...
    "После того, как азарт смыл с неё все наносное, под коркой хамства, чёрствости и зла оказалась очень приятная озорная девчонка, чем-то напоминающая Ульянку."
...
label alt_day2_slot_us:
    ...
    play sound sfx_soccer_ball_kick
    "С задорным смехом она врезала по невесть откуда взявшемуся у неё мячу и…"
    "Он полетел прямо в меня!"
    "Скорость у него была бешенная — ни уклониться, ни отскочить!"
...
label alt_day2_slot_dv:
    ...
    play music alice_theme fadein 3
    window hide
...
label alt_day2_slot_sl:
    ...
    show sl normal pioneer with dspr
    "Не то, чтобы для меня быть использованным было в новинку, не то, чтобы я не ожидал этого от Слави."
