...
label alt_day2_event_sport_area1:
    ...
    if ('un' in list_d2_convoy_7dl):
        ba "Чтобы хотя бы эта бадминтощица тебе не предпочла кого покрепче."
...
label alt_day2_dubstep:
    ...
    th "Микрофон!"
    if alt_day2_loki_minijack:    # нужно хоть пару строчек под else о том, как он походил по сцене и пошёл на обед. Иначе после фразы "Микрофон" Сёму слишком резко телепортирует на площадь.
        if ('sl' in list_d2_convoy_7dl):
