...
label alt_day2_bf:
    ...
    menu:
        ...
        "Рискнуть и сесть с рыжими." if alt_day_binder == 1:
            ...
            us "А что такое?"
            me "Да так."
            "Я крутнул тарелку по столу — в ней оставалось ещё половина порции." # крутанул
    ...
    menu:
        "Славя":
            ...
            if lp_sl >= 5:
                ...
                show sl laugh pioneer with dspr
                sl "Ты чего возмущаешься? Если не хочешь, не помогай."    # Надо сказать, что Славе нужна помощь на площади, а то не ясно, куда идти.
                $ list_d2_convoy_7dl = []
                $ list_d2_convoy_7dl.append('sl_prep')
                window hide
            else:
...
label alt_day2_inmusic:
    ...
    if ('un' in list_d2_convoy_7dl):
        ...
        "Стоящая рядом Славя чувствовала втихомолку улыбалась в кулачок." # не хватает союза какого-нибудь между "чувствовала" и "улыбалась".
    ...
    menu:
        "Записаться в кружок":
            ...
            if herc:
                ...
                show mi smile pioneer with dspr
                mi "Обязательно достану! Мне только на склад надо будет сходить, кажется, они были там, если хочешь, ты приходи после обеда, я постараюсь успеть, будем тогда играть вместе!"    # см. обед с Мику
...
label alt_day2_event_clubs1:
    ...
    if counter_sl_7dl >= 1:
        "В крайнем случае хотя бы штекер уворую и через усилитель послушаю."    # Герк вряд ли думает о том, чтобы уворовать штекер.
    else:
    ...
    if ('un' in list_d2_convoy_7dl):
        ...
        un "Опять Шурик где-то гуляет…" # при if ('library' in list_voyage_7dl) мы знаем, где он
        ...
        show dv angry pioneer2 with dspr
        dv "Ну и где этот очкарик?" # при if ('library' in list_voyage_7dl) мы знаем, где он
        me "Кто?"
...
label alt_day2_dubstep:
    ...
    if alt_day2_loki_minijack:
        if ('sl' in list_d2_convoy_7dl):
            ...
            th "Гусары, молчать!"
            th "Я имею в виду, она же несовершеннолетняя." # не уверен, но этот разговор Семёна с собой выглядит весьма странно. Мб вторую фразу должен произносить Шиза?
        ...
        "С аниме-девочкой":
            ...
            if 'music_club' in list_voyage_7dl:
                ...
                menu:
                    ...
                    "Молча сесть и заткнуться":
                        "Одарив её взглядом как рублём, я молча сел напротив." # фраза употребляется неправильно, в значении "уничтожила взглядом". https://phraseology.academic.ru/8766/
            ...
            menu:
                ...
                "А ты постоянно там в клубе сидишь?":
                    ...
                    show mi upset pioneer with dspr
                    if 'music' in list_clubs_7dl:
                        mi "Ой, извини, ты же уже записался. А ты на чём будешь играть? На гитаре?"    # Про гитару говорил только Герк. А Локи и Дрищу Мику пообещала мундштук достать.
                        me "Как дело пойдёт."
                        "Уклончиво ответил я."
                    else:
...
label alt_day2_eventEv_square1:
    ...
    if (lp_sl >= 8) and (counter_sl_cl == 1):
        ...
        "Мы не слишком-то поладили сегодня, поэтому её энтузиазм выглядел странновато." #это с 11-то поинтами Слави "не слишком поладили"? Да, Семён тут вскоре начинает аутировать и избегать Слави, но на данный момент вроде еще ничего подобного не было. И общались они вполне нормально.
...
label alt_day2_slot_sl:
    ...
    "Она так и не смотрела в мою сторону, вместо этого мечтательно улыбаясь и теребя косу." # На этой строке действует спрайт "sl tender pioneer". По-моему, здесь уместен скорее спрайт, где Славя улыбается и держится за косу (sl shy pioneer). 
