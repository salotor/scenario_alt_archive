label alt_day2_begin:
    ...
    if alt_day_binder != 1:
        ...
        me "А что…"
        if herc or loki:
            mt "Откроешь шкаф, там на плечиках висит комплект формы, будешь ходить в ней." # а вчера говорила mt "Форму тебе Славя завтра тоже даст, не волнуйся."
            me "Да нет, я…"
...
label alt_day2_bf:
    ...
    menu:
        "Сесть со Славей":
            ...
            sl "Может… Ты…"
            "Я уже говорил про непосредственную Славю, спортивную Славю, целеустремлённую Славю… {w}А теперь мне выпала уникальная возможность познакомиться со Славей кокетничающей." # на пути к 7дл уже выпадала, особенно у клубов
...
label alt_day2_dubstep:
    ...
    if alt_day2_loki_minijack:
        if ('sl' in list_d2_convoy_7dl):
            ...
            th "Гусары, молчать!"
            th "Я имею в виду, она же несовершеннолетняя." # им же 18
...
label alt_day2_dinner:
    ...
    if len(list_clubs_7dl) > 1: 
        ...
        th "Свят-свят!"
    # нужен вариант реплики ОД для == 1
    window hide
...
label alt_day2_siesta:
    ...
    th "То бишь, географических зацепок ноль, хронологических отметок… Славя что-то заикалась насчёт восемьдесят девятого… Мать моя женщина, тридцать лет без малого — в прошлое!" # всё очень плохо. 89 упоминается только в обычном 1 дне без рельсов, а прошлый 88 при рельсовом прохождении Славя упоминает только пр ипосещении спортплощадки на обходе. В остальных случаях год не упоминается.
...
label alt_day2_supper:
    ...
    if (counter_sl_7dl == 3):
        "А вскоре к её смеху присоединились и мы со Славей." # Семён уже погибал от хохота
    else:
...
label alt_day2_slot_sl:
    ...
    show sl normal pioneer
    with dissolve
    play music music_7dl["what_am_i_doing_here"] fadein 2 # музыка кмк к происходящему подходит не очень хорошо. Её бы с момента "Слово было сказано." запускать
    ...
    "Мы пришли к самой калитке." # а следующим кадром уже домики
    window hide
