...
label alt_day2_event_sport_area1:
    ...
    if ((alt_day1_sl_keys_took == 1) or (alt_day2_sl_guilty != 0)) and (alt_day2_rendezvous == 2):
        ...
        if alt_day2_sl_guilty != 0:
            ...
            me "Ну, я тогда сначала подпись у физрука возьму… Где он, кстати?" #Славя после получения подписи у физрука должна бы потащить Семёна на площадь
...
label alt_day2_grand_escape:
    ...
    me "Легче лёгкого. Главное — добраться до острова Длинного, там подобраться к насыпи и выждать. Если я правильно определил породу острова, он должен быть болотистым, а значит, машинист волей-неволей сбросит скорость. Тут-то мы и…" #Добраться - подобраться. Два однокоренных слова в одном предложении. 
    us "И запрыгнем!"
    ...
    "Ещё пять минут — и Ульяна спрыгнула на песок. Вслед за ней зашёл и я, предварительно втащив лодку поглубже на берег и примотав её к дереву." #Нарушена последовательность действий. Невозможно вытащить лодку, не сойдя с неё сначала.
    us "Что там дальше?"
...
label alt_day2_supper:
    ...
    th "Ха! Торт!"
    th "Для ничего не подозревающей вожатой она на подозрение быстро сориентировалась." #Два однокоренных слова в одном предложении. 
    ...
    "Впилась в одну из половинок зубами, отхватив сразу половину кремовой башенки!" #Два однокоренных слова в одном предложении. 
    mt "Ульяна!"
...
label alt_day2_eventEv_me_mt_house1:
    ...
    "Она всё ещё стояла, немного нависнув надо мной, касаясь — я окостенел."
...
label alt_day2_slot_un:
    ...
    "Сверху вниз силуэт читался с трудом, с уверенностью можно было сказать, что он не очень высокий."
