label alt_day2_begin:
    ...
    if alt_day_binder != 1:
        ...
        if herc or loki:
            ...
            mt "Откроешь шкаф, там на плечиках висит комплект, будешь ходить в ней."
            me "Да нет, я…"
    ...
    "Надо три."
    "Хотя бы."
    "Хотя пальцы уже ощутимо сводило от холода. Да хотя бы ещё одну."
...
label alt_day2_lineup:
    ...
    if counter_sl_7dl != 3:
        ...
        me "Пожалуйста."
        show mt surprise pioneer with dspr
        mt "Ладно, как скажешь. {w}Если сумеешь кого-нибудь уговорить, хорошо. А то давай Электроника?"    # Если сумеешь кого-нибудь уговорить — хорошо
        me "Нет!"
        show mt smile pioneer with dspr
    mt "И не забудь заглянуть в библиотеку — посмотришь, как рисует наша художница."    # me "Нет!" - mt "И не забудь заглянуть в библиотеку — посмотришь, как рисует наша художница." - Проблема в том, что в текущем варианте логическая привязка с напоминанием заглянуть в библиотеку выглядит слишком резкой, непонятной к чему.
    me "Это ещё зачем?"
    ...
    menu:
        "Сесть со Славей":
            ...
            if (counter_sl_7dl <= 2):
                ...
                show sl smile2 pioneer with dspr
                sl "Какой-нибудь раут? Бал? {w}Завтра будут танцы. Ты ходишь на танцы?"    # Семёну задали вопрос - где его реакция? Он молча в размышления ушёл.
            else:
            ...
            "Я тоже погрузился в какие-то свои размышления."    # Где эти самые размышления? Обыграть через многоточия и моргание экрана?
            window hide
            with fade2
            ...
            sl "Может… Ты…"
            "Я уже говорил про непосредственную Славю, спортивную Славю, целеустремлённую Славю… {w}А теперь мне выпала уникальная возможность познакомиться со Славей… Даже не знаю."
            "Кокетничающей?"
            if counter_sl_7dl > 1:
                extend " Только не так, как раньше."
            th "Ну и правильно, что она, робот, что ли."    # Ну и правильно. Что она, робот что ли?
            if herc or loki:
    ...
    menu:
        ...
        "Алиса":
            ...
            if lp_dv >= 3:
                ...
                if not (herc or loki) and (counter_sl_7dl == 0):
        ...
        "Лена" if not 'un_rej' in list_d2_convoy_7dl:
            ...
            elif not (herc or loki) and (counter_sl_7dl == 0):
...
label alt_day2_inmusic:
    ...
    "Просторное помещение, портреты Баха, Шуберта и Листа в рамочках на стенах, рояль, пюпитр под вокалиста, микрофон — здесь явно пели, и делали это с удовольствием."    # возможно фамилии композиторов стоит убрать под if loki? Герк и Дрищ связаны с музыкой, но не профессиональные музыканты, так, сходу, могли и не определить, кто изображён.
    ...
    if alt_day_binder == 1:
        ...
        mi "Ты Семён?"
        "При этом она нагнулась, и я ещё раз увидел…"    # Мику встала, спросила, затем нагнулась, затем подошла и только тогда подняла инструмент... Последовательность нарушена. Поменять строчки местами?
...
label alt_day2_mz:
    if ('un' in list_d2_convoy_7dl):
        ...
        $ meet('mz','Библиотекарша')
        mz "Ну и кто тут кричит?"
        "Раздался скрипучий голос."    # скрипучий голос
        mz "Если это опять чёртов Данька безобразит…"
    ...
    "Глаза угрожающе-жёлтого цвета, спрятанные за очками с неисчислимым количеством диоптрий."    # Насчёт количества диоптрий: в таком случае, её очки должны быть видны за километры, а не как сейчас — просвечивают, будто их и нет вовсе, только оправа.
    "Добавим сюда предельно сварливое выражение лица, и вот — библиотекарус вульгарис."    # сварливое выражение лица
    mz "Кто таков?"
    "Голос оказался под стать — скрипучий, со сварливыми хозяйскими нотками."    # скрипучий, со сварливыми хозяйскими нотками - Мы уже слышали её скрипучий голос... Зачем повторяться?
...
label alt_day2_event_estrade:
    ...
    "До обеда было ещё немного времени, поэтому я решил прогуляться к эстраде."    # здесь бы добавить if'ов на сопровождающих Сёму девочек, а то создаётся впечатление, что он один на эстраду пошёл, бросив девочку
    if ('dv' in list_d2_convoy_7dl):
        ...
        "Осмотревшись — никого вокруг замечено не было — я поднялся на сцену."    # "никого вокруг замечено не было" - опять же - он может быть не один
    stop music fadeout 3
    stop ambience fadeout 6
    window hide
    with fade
...
label alt_day2_dubstep:
    ...
    if alt_day2_loki_minijack:
        ...
        "Вообще эстрада производила впечатление! Огромная половинка десятиугольника шатром нависала над основой сценой, под самой крышей разместились три юпитера, а на столике у дальней стены стоял"
        if ('dv' in list_d2_convoy_7dl):
...
label alt_day2_event_square_1:
    ...
    if len(list_voyage_7dl) > 1:
        ...
        window hide
        with fade2
        "Славя не обманула, мы действительно справились быстро, не прошло и получаса, как мы сгрузили последний пакет с мусором в тележку и осмотрели поле боя."    # «Действие, пояснение, расширение пояснения» — с запятыми это выглядит как набор фраз, а не цельное осмысленное предложение. - «Славя не обманула: мы действительно справились быстро — не прошло и получаса...» ИЛИ «Славя не обманула — мы действительно справились быстро. Не прошло и получаса...»
        ...
        $ list_d2_convoy_7dl.remove('sl_prep')
        $ list_d2_convoy_7dl.append('sl')
...
label alt_day2_dinner:
    ...
    menu:
        "С аниме-девочкой":
            ...
            "Мику обладала одной замечательной возможностью — я с первых секунд будто знал её всю жизнь. Какого-то там смущения или запретных тем для нас уже не существовало. Я просто расслабился и наслаждался разговором."    # "замечательной особенностью" - Возможность Мику — светить трусиками направо-налево. Особенность Мику — раскрепощать человека своей болтовнёй.
            menu:
...
label alt_day2_siesta:
    ...
    th "Сами в свою очередь игнорируют такие, казалось бы, маловажные и несложные вопросы, как название ближайших населённых пунктов…"    # Семён скорее всего за это время не задал ни одного вопроса про географию. Надо бы либо изменить это, либо добавить вопросы ранее (в первую встречу с вожатой мб?)
    th "Да хотя бы эту чёртову речку — вот хоть кто-нибудь название знает?"    #
    th "То бишь, географических зацепок ноль."    #
    if counter_sl_7dl < 2:
        th "Хронологических отметок… Славя что-то заикалась насчёт восемьдесят девятого… Мать моя женщина, тридцать лет без малого — в прошлое!"
        th "Но можно ли ей верить?"
        dreamgirl "Верить никому нельзя. Никому. Даже себе. Мне — можно."
        th "А с тобой вообще разговор особый! Что за подозрительный тип тут у меня ошивается в голове? Сквоттер или мозговой слизень с планеты Слизеринус?"    # Диалог с Шизой начинается... И тут же заканчаивается. Никаких «Ответа не последовало» или «Я мысленно отмахнулся рукой». Просто итог того, что было надумано перед этим «диалогом», как будто его и не было.
...
label alt_day2_tournament:
    ...
    hide us
    hide dv
    with dissolve    # стоит добавить к перечислению (и показать спрайты) тех, кого мы встретили на обходе - Шурика, Мику, Женю
    "Было и несколько незнакомых лиц."    # убрать под if not ('library' in list_voyage_7dl) or not (('music_club' in list_voyage_7dl) or alt_day2_mi_met), иначе Мику, Шурика и Женю мы уже видели, незнакомых лиц нет.
    ...
    "Удивительно, но Ольга просто спокойно улыбнулась мне."    # улыбающающийся спрайт Ольги
    show mt normal  pioneer at right with dspr
...
label alt_day2_un_herc_date:
    ...
    if ('nwsppr' in list_clubs_7dl) and alt_day1_un_dated:
...
label alt_day2_slot_dv:
    if not (alt_day2_dv_chased and lp_dv > 3 and alt_day2_dv_bet_approve):
        ...
        stop music fadeout 1
        play ambience ambience_lake_shore_evening fadein 2
        play music music_7dl["shestheone"] fadein 2
    ...
    window hide
    stop ambience fadeout 1
    play music music_7dl["sheiscool"] fadein 1
