...
label alt_day2_eventEv_square1:
    ...
    if (lp_sl >= 8) and (counter_sl_cl == 1):
        ...
        if alt_day1_sl_keys_took == 1:
            ...
            me "Ключи?"
        sl "Я… Связку потеряла. Извини."    # если отдали ключи Ольге - это КРАЙНЕ странно, она должна была вернуть уже их Славе
...
label alt_day2_slot_sl:
    ...
    $ set_mode_nvl()
    if herc:
        "Мне двадцать два… Белые питерские ночи, я сижу на подоконнике своей квартиры, выходящей окнами на бывшую церковь, из которой Советская Власть сделала бассейн."    # 22 - многовато, с остальными датами не стыкуется. Что-то около 18 должно быть?
