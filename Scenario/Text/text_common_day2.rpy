...
label alt_day2_lineup:
    ...
    "И вот наконец верхняя точка. {w=0.5}{nw}"
    play sound sfx_7dl["eat_horn"] fadein 1
    extend "И будто ожидая этого, жестяной рупор заиграл сигнал горна, священный для любого голодного пионера."
    ...
    if counter_sl_7dl != 3:
        ...
        me "Пожалуйста."
        show mt surprise pioneer with dspr
        mt "Ладно, как скажешь. {w}Если сумеешь кого-нибудь уговорить — хорошо."
    ...
    menu:
        "Сесть со Славей":
            ...
            if (counter_sl_7dl <= 2):
                ...
                show sl smile2 pioneer with dspr
                sl "Какой-нибудь раут? Бал? {w}Завтра будут танцы, и…"
            else:
...
label alt_day2_event_library1:
    ...
    if ('me' in list_d2_convoy_7dl) or ('dv_prep' in list_d2_convoy_7dl) or ('sl_prep' in list_d2_convoy_7dl):
        "Прикинув, что до библиотеки отсюда ближе всего, я развернулся на север."    # это фраза неуместна, учитывая что в библиотеку можно прийти ИЗ ЛЮБОЙ точки лагеря. От клубов она не "ближе всего", от муз.кружка она не на севере, а на востоке и т.д. Нужна более общая фраза
    else:
    ...
    if ('un' in list_d2_convoy_7dl):
        ...
        if loki:
            ...
            un "Да… Опять Шурик с Даней о газете спорят."    ## при этом, когда мы входим в редакцию стенгазеты, там находятся только Эл и Шурик. Лена ошиблась?
            "Прошептала Лена."
    ...
    menu:
        "Записаться":
            ...
            me "Точно нет? Я вижу, вас тут всего трое плюс девочка. Кто материал-то собирает?"    ## в помещении находятся только Эл и Шурик. Про то, что Даня тоже входит в состав, говорит Лена только если идёт с нами и только Герку с Локи. В остальных случаях Сёма не знает про то, что в стенгазете "ТРОЕ плюс девочка"
...
label alt_day2_dubstep:
    ...
    if alt_day2_loki_minijack:
        ...
        "Вообще эстрада производила впечатление! Огромная половинка десятиугольника шатром нависала над основной сценой, под самой крышей разместились три юпитера, а на столике у дальней стены стоял"
        if ('dv' in list_d2_convoy_7dl):
...
label alt_day2_siesta:
    ...
    th "Сами в свою очередь игнорируют такие, казалось бы, маловажные и несложные вопросы, как название ближайших населённых пунктов…"    #### Сёма пытается спрашивать про географические названия только в 1 месте - если во время обхода зашёл на пляж без Слави, он спрашивает у Ольги. В остальных случаях строчки про "игнорируют вопросы..." неуместны
    ...
    if ('un' in list_d2_convoy_7dl):
        ...
        window hide
        scene anim prolog_1
        stop music fadeout 3    # здесь наоборот надо какую-то музыку включить, так как до этого ничего не играло.
...
label alt_day2_tournament:
    ...
    with dissolve
    us "То есть, два!"    ### Алиса с Ульяной УЖЕ стоят у столовой, когда приходит Семён. А в некоторых случаях (см. выше) Улька уже обсуждает возможность переигровок (а значит собирается играть). Довольно странно, что они присоединяются только сейчас. Разве что, после Великого Побега, они приходят вместе с Сёмой. В противном случае - рыжие там уже стояли и не записывались?
    ...
    sl "Карточный турнир."
    "Обернулась Славя."
    sl "Нам не хватало участников, поэтому ты, Алиса и Ульяна очень вовремя!"    ### см. выше - Алиса и Ульяна УЖЕ стояли там (кроме Великого Побега).
