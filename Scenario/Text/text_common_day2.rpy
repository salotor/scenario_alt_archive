...
label alt_day2_bf:
    ...
    menu:
        ...
        "Рискнуть и сесть с рыжими" if persistent.7dl_binder:
...
label alt_day2_dinner:
    ...
    if (persistent.7dl_binder) and not 'music_club' in list_voyage_7dl:
...
label alt_day2_tournament:
    ...
    elif (persistent.7dl_binder) and not ('library' in list_voyage_7dl):
        show el normal pioneer with dissolve
        $ meet('el','Электроник')
...
label alt_day2_walk_sl:
    ...
    if not herc:
        ...
        window hide
        scene bg ext_house_of_mt_day with dissolve
        "Крейсерская скорость Слави отличалась от скорости крайне незначительно — так что перемещения по лагерю обычно занимали считанные минуты."
