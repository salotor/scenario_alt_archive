        ...
        menu:
            "Спросить о штекере":
                ...
                if counter_sl_cl == 1:
                    ...
                    if not (herc or loki):    # здесь уместнее проверка на not ('library' in list_voyage_7dl). Т.к. если ('library' in list_voyage_7dl), то разговор закончился на фразе "Бегунок у меня, помнишь?", иначе - на рукопожатии с Шуриком. И тут как раз уместен вопрос про бегунок. Кроме того, проверка на Дрища тут не важна - у него и Герка одни и те же фразы. А Локи сюда не попадает и вовсе, ибо молча тырит штекер.
                        sh "Бегунок?"
                        me "Угу."
                    else:
