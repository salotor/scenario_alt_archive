label alt_day2_begin:
    ...
    play sound sfx_open_door_kick #Ы
    with vpunch #Ы
    pause(1) #Ы
    window hide
...
label alt_day2_dinner:
    ...
    show us angry pioneer at cleft with dissolve #Ы
    $ renpy.pause(1)
    show us angry pioneer at cright with move #Ы
    ...
    show us smile pioneer at center with dissolve #Ы
    us "А как ты такую штуку со стаканом сделал? Покажи, а?"
    me "Умоешься, тогда скажу."
    show us dontlike pioneer with dspr #Ы
...
label alt_day2_tournament:
    ...
    with dissolve
    mi "А где наш новенький? Семён. Сеня?"    ## уместно только если мы познакомились с Мику до этого момента (('music_club' in list_voyage_7dl) or alt_day2_mi_met)
...
label alt_day2_eventEv_square1:
    ...
    th "Может, разделить с ним тортик?"    ## нужна проверка на alt_day2_dv_chased. Сёма явно убегал от Алисы без тортика, тот бы такого обращения не пережил никак.
    menu:
        "Почему нет?":
            "…"
            th "Ммм… Вкуснотища!"
...
label alt_day2_eventEv_boat_station1:
    ...
    th "Сесть на берегу и насладиться тортиком?"    ## нужна проверка на alt_day2_dv_chased. Сёма явно убегал от Алисы без тортика, тот бы такого обращения не пережил никак.
    menu:
        "Конечно":
            "…"
            th "Ммм… Вкуснотища!"
...
label alt_day2_un_loki_date:
    ...
    th "Не смешно."
    "В любом случае, я уселся на пресловутую скамеечку и, неспешно поедая остатки торта, принялся ждать."    ## нужна проверка на alt_day2_dv_chased. Сёма явно убегал от Алисы без тортика, тот бы такого обращения не пережил никак.
    ...
    with dissolve
    "Наконец торт был благополучно приговорён, а в домике так ничего не изменилось."    ## нужна проверка на alt_day2_dv_chased. Сёма явно убегал от Алисы без тортика, тот бы такого обращения не пережил никак.
...
label alt_day2_slot_dv:
    ...
    window hide
    stop ambience fadeout 1
    play music music_7dl["sheiscool"] fadein 1    # музыка не подходит. Старая БЛовская тема Алисы здесь была уместна, новая - нет. У ней другой настрой
