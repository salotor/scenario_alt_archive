label alt_day0_start_d:
    ...
    "Я не был и не ночным жителем, и дневным меня назвать было трудно."
    stop sound_loop
    ...
    "И верилось: во всём мире теперь только так — промозгло, холодно и приглушенно-шумно."
    ...
    "В тот самый момент, когда «сто вторая» маршрутка хлопнула за спиной дверью и умчалась прочь, оставляя меня наедине с надписью «Пискарёвское мемориальное»…"
    "И памятью."
    ...
    dreamgirl "Последний логический шаг,{w} Сём. Признай, что и тебя — тоже нет."
    me "И меня тоже нет."
    ...
    dreamgirl "А «лансер» — это от английского названия лёгкого пехотного копья."
    ...
    dreamgirl "А может нет? Может, это не фрилансер, а ланс-фри?{w} Человек, свободный от тягот службы и ношений всяких длинных тяжёлых штук?"
    ...
    dreamgirl "Тебе тоже в серебрениках платят?{w} Кстати, всегда было любопытно, а тридцать серебреников — это сколько по нынешнему курсу доллара?"
    th "Заткниииись."
...
label alt_day0_start_h:
    ...
    "А мой отец, тот, биологический, привил мне тягу к этой Высшей Справедливости — и это было его единственным вкладом в моё воспитание."
...
label alt_day0_d3_prologue:
    ...
    window hide
    with fade
    "Декорации за спиной девушки сместились, вспыхнул яркий свет, а когда я проморгался…"    # тут уместнее flash. Да и scene убрать бы (заменить на scene white, пока он ослеплён?)
    ...
    dreamgirl "Сеееень?.."
    "Она была настоящая."
    "Я почувствовал, будто проваливаюсь в какую-то бездонную яму, с тёмными трепещущими краями, состоящую из единого восторга."
    ...
    "Потом долго пытался успокоить запалённое дыхание."
    me "Что за дела?.."    # am?
    ...
    "Потом зазвонил будильник, и я, мгновенно собравшись, выскочил на улицу."    # вот только здесь пора менять бг на хрущёвку. До этого уместнее экран
    ...
    "Вот только её изготовление требует совсем других навыков, а не молниеносной реакции и точности с контролем."
    ...
    "{i}Просто надеялся на то, что всё сложится как-то иначе?..{/i}"
    "{i}Я не знаю.{/i}"
    ...
    "Тем более, я видел: новая-старая дисциплина привлекает всё новых и новых инвесторов, глупые стрелялки из детства превращаются в хорошо оплачиваемое шоу."
    ...
    "Я сохранил текст на рабочем столе, запаролил компьютер и отправился на боковую."    # сменить экран монитора на потолок квартиры?
    "У меня всё ещё остались мои сны."    # blink?
    $ night_time()
    play ambience ambience_camp_center_night fadein 3
    stop music fadeout 3
    ...
    show expression D3_intro("bg ext_entrance_night_clear_closed_7dl")    # оно не сменяет предыдущий scene, а показывается вместе с ним. Надо скрыть старый перед вызовом
    show owl behind prologue_dream:
        pos (931, 88)
    show prologue_dream
    with fdiam
