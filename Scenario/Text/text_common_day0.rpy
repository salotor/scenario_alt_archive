...
label alt_day0_start_h:
    ...
    "Так что вместо завтрака я убирал за Махой, снова чего-то наевшейся и, как результат…"    # стоит вернуть в текст упоминание, чего именно наелась Маха (ватных палочек), иначе фраза в Сл-7дл "почти открытая пластиковая коробка с ватными палочками — видимо, этим и развлекался мой домашний питомец, пока я отсутствовал" теряет смысл
    "Маха — это моя кошка."
    "А я — дурак."
    window hide
    scene anim intro_1 
...
label alt_day0_start_l:
    ...
    with fade
    play sound_loop sfx_street_traffic_outside fadein 2
    "Во мне что-то сломалось. Куда-то делся внутренний яростный огонь, что мог бы сделать меня карающей дланью, толкнуть на преступление, на глупости."    # redemption отсюда и далее не подходит. Под сломанного Локи и его месть хорошо звучала старая beth. Лучше с этого момента переключить redemption на beth
...
label alt_day0_d3_prologue:
    ...
    window hide
    scene cg d1_uv_bus_7dl with fade
    pause(.1)
