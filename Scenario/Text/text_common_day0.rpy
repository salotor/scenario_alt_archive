label alt_day0_start_d:
    ...
    if jopa:
        "Йей, вылет благополучно заборот!"
    else:
        "Вылет? Вылет!"
...
label alt_day0_start_l:
    ...
    me "Конечно же, нет! Я даже не собираюсь…" # Логичнее сделать это th, раз уж далее «Чего я не собирался — я не договорил».
...
label alt_day0_d3_prologue:
    ...
    "Несмотря даже на то, что нас представили друг другу общие знакомые спустя неделю."
    "А ещё через неделю я признался своему единственному другу — тому таинственному собеседнику из сети — в том, что, кажется, схожу с ума."
    "И полтора часа восторженных эпитетов, расписываний того «какая она!» спустя, мне задали вопрос: и что, нашему общению теперь конец?" # Имхо, нужны запятые вокруг «какая она!».
