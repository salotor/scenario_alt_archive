            ...
            if (us_pt > 0) or (mt_pt > 0) or (d3_pt > 0):
                ...
                if lp_mi < 13:
                    ...
                    if alt_day3_mi_date and 'mi' in list_d2_date_7dl and ((alt_day3_dancing == 41) or (alt_day3_dancing == 40)):
                        ui.text("%s: %d" % ("Мику_7ДЛ", lp_mi), style="button_text", size=14, color="#00bbbb")
                        if (alt_spt >= 8) or ((alt_hpt >= 9) and (lp_mi > 16)):
                            ui.text("%s: %d" % ("Мику_7ДЛ", lp_mi), style="button_text", size=15, color="#00ffff")
                    elif (lp_mi >= 13) and alt_day3_mi_dj:
                        ui.text("%s: %d" % ("Мику_DJ", lp_mi), style="button_text", size=14, color="#00bbbb")
                        if alt_day6_mi_dj_hentai2 or alt_day6_mi_dj_no_hentai:
                            ui.text("%s: %d" % ("Мику_DJ", lp_mi), style="button_text", size=15, color="#00ffff")
                ...
                if (lp_sl < 13) and (counter_sl_cl == 0) and (counter_sl_7dl == 0):
                    ...
                    if (counter_sl_cl < 5) and (counter_sl_7dl < 5):
                        ...
                        if ((lp_sl >= 20) or (alt_sp >= 6)) or ((lp_sl >=20) and (alt_day6_sl_arc == 1)):
                            ui.text("%s: %d" % ("Славя_КЛ", lp_sl), style="button_text", size=15, color="#ffaa00")
                        else:
                            ui.text("%s: %d" % ("Славя_КЛ", lp_sl), style="button_text", size=14, color="#bbb200")
                    elif counter_sl_7dl == 5:
                        if (lp_sl >= 20) and (karma > 120):
                            ui.text("%s: %d" % ("Славя_7ДЛ", lp_sl), style="button_text", size=15, color="#ffaa00")
                        if (lp_sl >= 20):
                            ui.text("%s: %d" % ("Славя_7ДЛ", lp_sl), style="button_text", size=15, color="#bbb200")
                        else:
                            ui.text("%s: %d" % ("Славя_7ДЛ", lp_sl), style="button_text", size=14, color="#bbb200")
